...
def source_name_as_path(generator):
    ...
    return os.path.normpath(os.path.join(dir, f"{generator.get_hash()}{ext}"))
...
def source_name_dot_hash(generator):
    ...
    if source_filename is None or os.path.isabs(source_filename):
        ...
        # use the source's name to create the new filename.
        dir = os.path.join(
            settings.IMAGEKIT_CACHEFILE_DIR, os.path.dirname(source_filename)
        )
...
def hash(generator):
    ...
    return os.path.normpath(
        os.path.join(settings.IMAGEKIT_CACHEFILE_DIR, f"{generator.get_hash()}{ext}")
    )
